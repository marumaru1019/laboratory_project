...
class _Standard:
    ...
    # dictをインスタンスとして定義
    def __init__(self, df_dict, df_dict_moves, df_dict_diffs,cols):
...
class _CreateDataset(_Standard):
    ...
    def make_data_c(self,concat):
        colms = concat.shape[1]-2
        step = int(colms/2)
        ex1 = concat.iloc[:,0:step]
        ex2 = concat.iloc[:,step:step*2]
        ex1 = np.array(ex1).reshape(-1,step,1)
        ex2 = np.array(ex2).reshape(-1,step,1)
        fin = np.concatenate([ex1,ex2], axis=2)
        pk = concat.iloc[:,-2]
        pk = np.array(pk)
        pk = pk.reshape(-1,1)
        label = concat.iloc[:,-1]
